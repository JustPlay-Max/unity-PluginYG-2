...
namespace YG.Insides
{
    public partial class AdvCallingSimulation
    {
        public static void InterstitialAdvOpen()
        {
            ...
            call.StartCoroutine(call.InterstitialAdvOpen(YG2.infoYG.simulationInEditor.durationAdv));
        }

        public IEnumerator InterstitialAdvOpen(float duration)
        {
            yield return new WaitForSecondsRealtime(YG2.infoYG.simulationInEditor.loadAdv);
            YGInsides.OpenInterAdv();

            if (!YG2.infoYG.simulationInEditor.testFailAds)
