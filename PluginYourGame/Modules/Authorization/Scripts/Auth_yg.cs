...
namespace YG
{
    public partial class YG2
    {
        ...
        {
            player.auth = infoYG.simulationInEditor.authorized;
            player.id = infoYG.simulationInEditor.uniqueID;
            player.payingStatus = infoYG.simulationInEditor.payingStatus;

            if (!infoYG.simulationInEditor.authorized)
            {
            ...
            else
            {
                if (!infoYG.Authorization.scopes)
                    ...
                    player.name = infoYG.simulationInEditor.playerName;
            }

            if (infoYG.simulationInEditor.playerPhoto == InfoYG.DEMO_IMAGE)
                player.photo = ServerInfo.saveInfo.playerImage;
            else
                player.photo = infoYG.simulationInEditor.playerPhoto;
